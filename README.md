...
**Latest Enhancements (v0.2.1)**: This version introduces comprehensive batch processing capabilities with intelligent Chinese chapter parsing, enhanced text processing features, and advanced file management. Key additions include smart chapter detection supporting multiple Chinese formats (第X章, 卷X, 序言, (一)章节), batch file conversion with unified output management, encoding detection with chardet library, and text cleaning functions. The system now supports processing multiple files simultaneously with progress tracking and task management.
